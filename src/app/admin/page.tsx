...
export default async function AdminDashboard({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  // Client-side-ish filters for work_type/status on entries
    ...
    <div className="space-y-4">
      ...
      <form className="card p-4 grid md:grid-cols-6 gap-3 text-sm">
        ...
        <div className="md:col-span-6 flex gap-2">
          ...
          <Link href="/admin" className="btn-secondary text-sm">Reset</Link>
        </div>
      </form>

      <div className="card p-4 flex items-center justify-between">
        <span className="text-stone-600">{filtered.length} time card(s)</span>
      ...
      <div className="space-y-2">
        ...
        {filtered.length === 0 && <p className="text-stone-600">No time cards match these filters.</p>}
      </div>
    </div>
  );
}
